        ...
        <div class="max-w-6xl mx-auto space-y-8 px-4 sm:px-6 lg:px-8 pt-24 pb-12" v-if="user">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-2xl font-display font-bold text-slate-900 dark:text-white tracking-tight">Ajout d'un livre</h2>
                    </div>
                    <p class="text-sm text-slate-500 dark:text-slate-200 mt-1">Ajoutez un nouveau livre à votre bibliothèque !</p>
                </div>
            </div>
            <form class="flex flex-col gap-4" @submit.prevent="changeData">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <input type="file" name="image" id="image" ref="file" required class="w-full text-sm outline-none border border-slate-300 dark:border-slate-200 bg-slate-50 rounded-md p-2" @change="onFileChange">
                </div>
                ...
                <div class="mt-2 flex flex-col md:flex-row md:justify-end">
                    <button class="px-4 lg:px-8 py-2.5 bg-orange-600 dark:bg-orange-500 dark:hover:bg-orange-600 text-white text-sm rounded-md hover:bg-orange-700 transition-colors">Valider</button>
                </div>
            </form>
        </div>
    </div>
...
const changeData = async () => {
    error.value = null;
    message.value = null;
    if(name.value.trim() !== "" && !image.value && description.value.trim() !== "") {
        const payload = {
        }
