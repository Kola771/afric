    ...
    <div class="bg-[#fffcfccc] dark:bg-dark">
        <div class="max-w-6xl mx-auto space-y-8 px-4 sm:px-6 lg:px-8 pt-24 pb-12" v-if="user">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-2xl font-display font-bold text-slate-900 dark:text-white tracking-tight">Modification du livre : "<strong class="text-orange-600 dark:text-orange-500">L'amour et la trahison</strong>"</h2>
                    </div>
                    <p class="text-sm text-slate-500 dark:text-slate-200 mt-1">Modifiez les informations de votre livre !</p>
                </div>
            </div>
            <form class="flex flex-col gap-4" @submit.prevent="changeData">
                ...
                <div class="flex flex-col gap-1">
                    <label for="name" class="text-sm text-slate-900 font-medium dark:text-white">Titre du livre :</label>
                    <input type="text" id="name" v-model="name" class="block w-full rounded-lg border-0 p-2.5 lg:p-2 text-slate-900 shadow-sm border-slate-300 border-[1px] placeholder:text-slate-400 focus:ring-2 outline-none dark:focus:ring-orange-500 focus:ring-orange-600 text-sm sm:leading-6 transition-all" />
                </div>
                <div class="flex flex-col gap-1">
                    <label for="image" class="text-sm text-slate-900 font-medium dark:text-white">Image de couverture :</label>
                    <input type="file" name="image" id="image" ref="file" class="w-full text-sm outline-none border border-slate-300 dark:border-slate-200 bg-slate-50 rounded-md p-2" @change="onFileChange">
                </div>
                ...
                <div class="flex flex-col gap-1">
                    <label for="description" class="text-sm text-slate-900 font-medium dark:text-white">Description du livre :</label>
                ...
                <div>
                    <label for="step" class="text-sm text-slate-900 font-medium dark:text-white">État de l'histoire :</label>
                    <select required id="step" class="w-full text-sm placeholder:text-slate-500 text-slate-800 outline-none border border-slate-300 dark:border-slate-200 dark:bg-slate-50 rounded-md p-2.5" v-model="step">
...
const changeData = async () => {
    error.value = null;
    message.value = null;
    if(name.value.trim() !== "" && step.value.trim() !== "") {
        const payload = {
        }
