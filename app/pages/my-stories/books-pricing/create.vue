<template>
    <div class="bg-[#fffcfccc] dark:bg-dark dark:border-slate-200 dark:border-b">
        <div class="max-w-6xl mx-auto space-y- px-4 sm:px-6 lg:px-8 pt-24 pb-12" v-if="user">
            <!-- Welcome Section -->
            <div class="flex flex-col gap-6">
                ...
                <div
                    ...
                    <ul class="text-sm text-slate-500 dark:text-slate-200 mt-2 list-disc pl-5 space-y-1">
                        <li>Sélectionnez avec attention le livre correspondant à votre configuration.</li>
                        <li>Assurez-vous de disposer des droits nécessaires pour bénéficier des revenus liés à ce
                            contenu.</li>
                    </ul>
                </div>
            </div>
            <form class="flex flex-col gap-4" @submit.prevent="createBook">
                ...
                <div class="mt-2 flex flex-col md:flex-row md:justify-end">
                    <button :disabled="loading"
                        ...
                        class="px-4 lg:px-8 py-2.5 bg-orange-600 dark:bg-orange-500 dark:hover:bg-orange-600 text-white text-sm rounded-md hover:bg-orange-700 transition-colors">Valider
                        cette configuration</button>
                </div>
            </form>
        </div>
...
const { createBooksPricing } = booksPricingData();
...
const createBook = async () => {
    ...
    try {
        ...
        if (pdf_price.value > 0 && subscription_price_monthly.value > 0 && id_book.value && currency.value.trim() !== "") {
            const res = await createBooksPricing({book: {id: id_book.value}, pdf_price: pdf_price.value, subscription_price_monthly: subscription_price_monthly.value, currency: currency.value})
            if (res.success) {
