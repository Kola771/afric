    ...
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h2 class="text-xl font-display font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
                    ...
                    <span>Modification d'une catégorie : "<strong class="text-orange-600 dark:text-orange-500" v-if="category">{{ category?.name }}</strong>"</span>
                </h2>
        ...
        <form class="flex flex-col gap-2.5" @submit.prevent="updateCategory">
            <div class="flex flex-col gap-1">
                ...
                <div class="bg-slate-100 dark:bg-slate-800 flex flex-col rounded-lg"><img :src="`${config.public.apiBackendUrl}/uploads/categories/${category?.image}`" v-if="category" alt="Image actuelle" class="max-h-[200px] dark:border object-cover lg:object-contain lg:max-h-[220px] rounded-lg" /></div>
            </div>
            <div class="flex flex-col gap-1">
                <label for="name_category" class="text-sm text-slate-900 font-medium dark:text-white">Nom de la catégorie :</label>
                <input type="text" id="name_category" name="name_category" v-model="name_category" required class="block w-full rounded-lg border-0 p-2.5 lg:p-2 text-slate-900 shadow-sm border-slate-300 border-[1px] placeholder:text-slate-400 focus:ring-2 outline-none dark:focus:ring-orange-500 focus:ring-orange-600 text-sm sm:leading-6 transition-all" />
            </div>
            <div class="flex flex-col gap-1">
