<template>
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <form class="flex flex-col gap-2.5" @submit.prevent="updateRole">
...
<script setup lang="ts">
    ...
    const { getRoleByUuid, updateData } = rolesData();
    ...
    onMounted(async () => {
        const { data } = await getRoleByUuid(`${route.params.uuid}`);
        role.value = data;
        if(role.value) {
    ...
    const updateRole = async () => {
        ...
        if(name.value.trim() !== "" && description.value.trim() !== ""){
            const res = await updateData(`${route.params.uuid}`, {name: name.value, description: description.value})
            if(res.success) {
