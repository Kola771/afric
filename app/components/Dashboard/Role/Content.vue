    ...
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="flex gap-3">
                <button @click="exportRoles" class="h-9 px-4 rounded-lg bg-white dark:bg-slate-100 border border-slate-200 text-xs font-medium text-slate-700 hover:bg-slate-50 shadow-sm transition-colors flex items-center gap-2">
        ...
        <div class="bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
            ...
            <div class="flex items-center gap-3 w-full">
                <div class="relative flex-1">
                    ...
                    <input type="search" v-model="searchQuery" @keydown.enter="executeSearch" placeholder="Rechercher un rôle..." class="h-8 pl-8 pr-3 w-full rounded-lg bg-slate-50 border border-slate-200 text-xs focus:outline-none focus:ring-1 focus:ring-slate-300 transition-colors">
                </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="overflow-x-auto lg:max-h-96 lg:overflow-y-auto custom">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-sm">
                        <tr class="group hover:bg-slate-50 transition-colors border-b border-slate-50" v-for="(role, index) in sortedRoles" :key="index">
                            ...
                            <td class="py-3 px-6 whitespace-nowrap">
                                <span :class="`capitalize inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-medium ${role.status === 'actif' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'} `">
                                    <span :class="`w-1 h-1 rounded-full ${role.status === 'actif' ? 'bg-green-600' : 'bg-red-600'}`"></span>
                                    {{ role.status }}
                                </span>
                            </td>
                            <td class="py-3 px-6 whitespace-nowrap">
                                <span :class="`inline-flex items-center gap-1.5 text-[10px] font-medium ${role.status === 'actif' ? ' text-green-700' : 'text-red-700'} `">
                                    <span :class="`w-1 h-1 rounded-full ${role.status === 'actif' ? 'bg-green-600' : 'bg-red-600'}`"></span>
                                    {{ formatLocalDate(role.deadline || '') }}
                                </span>
                            </td>
                            <td class="py-3 px-6 text-right whitespace-nowrap">
                                <nuxt-link :to="`/dashboard/roles/edit/${role.uuid}`" v-if="role.status === 'actif'" class="ml-2 text-orange-600 hover:text-orange-700 transition-colors"><Icon name="mdi:edit" class="w-4 h-4" /></nuxt-link>
                                <button @click="toggleDeleteModal(role)" v-if="role.status === 'actif'" class="ml-2 text-red-600 hover:text-red-700 transition-colors"><Icon name="mdi:trash" class="w-4 h-4" /></button>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <DashboardRoleDelete @close-delete-modal="closeDeleteModal" :role="role" :showDeleteModal="showDeleteModal" v-if="role && showDeleteModal" />
    </div>
</template>
