    ...
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            <div class="bg-white dark:bg-transparent dark:border dark:border-slate-200 dark:text-slate-200 p-4 rounded-xl border border-slate-200 shadow-sm">
                <p class="text-xs font-medium text-slate-500 dark:text-slate-300 mb-1">Total Auteurs</p>
            ...
            <div class="bg-white dark:bg-transparent dark:border dark:border-slate-200 dark:text-slate-200 p-4 rounded-xl border border-slate-200 shadow-sm">
                <p class="text-xs font-medium text-slate-500 dark:text-slate-300 mb-1">Auteurs Actifs</p>
                <div class="flex items-end justify-between">
                    <p class="text-2xl font-display font-bold text-slate-900 dark:text-slate-200">{{ formatNumber(countAllActifs) }}</p>
                    <Icon name="solar:users-group-two-rounded-linear" class="text-green-600" width="24" />
                </div>
            </div>
            <div class="bg-white dark:bg-transparent dark:border dark:border-slate-200 dark:text-slate-200 p-4 rounded-xl border border-slate-200 shadow-sm">
                <p class="text-xs font-medium text-slate-500 dark:text-slate-300 mb-1">Auteurs Inactifs</p>
                <div class="flex items-end justify-between">
                    <p class="text-2xl font-display font-bold text-slate-900 dark:text-slate-200">{{ formatNumber(countAllInactifs) }}</p>
                    <Icon name="solar:users-group-two-rounded-linear" class="text-red-500" width="24" />
                </div>
            </div>
            <div class="bg-white dark:bg-transparent dark:border dark:border-slate-200 dark:text-slate-200 p-4 rounded-xl border border-slate-200 shadow-sm">
                <p class="text-xs font-medium text-slate-500 dark:text-slate-300 mb-1">Auteurs Suspendus</p>
                <div class="flex items-end justify-between">
                    <p class="text-2xl font-display font-bold text-slate-900 dark:text-slate-200">{{ formatNumber(countAllSuspendus) }}</p>
                    <Icon name="solar:users-group-two-rounded-linear" class="text-purple-600" width="24" />
                </div>
            </div>
            <div class="bg-white dark:bg-transparent dark:border dark:border-slate-200 dark:text-slate-200 p-4 rounded-xl border border-slate-200 shadow-sm">
                <p class="text-xs font-medium text-slate-500 dark:text-slate-300 mb-1">Auteurs Bannis</p>
                <div class="flex items-end justify-between">
                    <p class="text-2xl font-display font-bold text-slate-900 dark:text-slate-200">{{ formatNumber(countAllBannis) }}</p>
                    <Icon name="solar:users-group-two-rounded-linear" class="text-red-600" width="24" />
                </div>
            </div>
            <div class="bg-white dark:bg-transparent dark:border dark:border-slate-200 dark:text-slate-200 p-4 rounded-xl border border-slate-200 shadow-sm">
                <p class="text-xs font-medium text-slate-500 dark:text-slate-300 mb-1">Auteurs certifiés</p>
                <div class="flex items-end justify-between">
                    <p class="text-2xl font-display font-bold text-slate-900 dark:text-slate-200">{{ formatNumber(countAllCertifies) }}</p>
                    <span class="w-2 h-2 rounded-full bg-orange-600 animate-pulse"></span>
                </div>
            </div>
            <div class="bg-white dark:bg-transparent dark:border dark:border-slate-200 dark:text-slate-200 p-4 rounded-xl border border-slate-200 shadow-sm">
                <p class="text-xs font-medium text-slate-500 dark:text-slate-300 mb-1">Best auteurs</p>
                <div class="flex items-end justify-between">
                    <p class="text-2xl font-display font-bold text-slate-900 dark:text-slate-200">{{ formatNumber(countAllBest) }}</p>
                    <span class="w-2 h-2 rounded-full bg-purple-600 animate-pulse"></span>
                </div>
            </div>
            <div class="bg-white dark:bg-transparent dark:border dark:border-slate-200 dark:text-slate-200 p-4 rounded-xl border border-slate-200 shadow-sm">
                <p class="text-xs font-medium text-slate-500 dark:text-slate-300 mb-1">Top auteurs</p>
                <div class="flex items-end justify-between">
                    <p class="text-2xl font-display font-bold text-slate-900 dark:text-slate-200">{{ formatNumber(countAllTop) }}</p>
                    <span class="w-2 h-2 rounded-full bg-green-600 animate-pulse"></span>
                </div>
            </div>
            <div class="bg-white dark:bg-transparent dark:border dark:border-slate-200 dark:text-slate-200 p-4 rounded-xl border border-slate-200 shadow-sm">
                <p class="text-xs font-medium text-slate-500 dark:text-slate-300 mb-1">Standard auteurs</p>
                <div class="flex items-end justify-between">
                    <p class="text-2xl font-display font-bold text-slate-900 dark:text-slate-200">{{ formatNumber(countAllStandard) }}</p>
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto">
                <div class="relative flex-1 sm:flex-none">
                    ...
                    <input type="search" placeholder="Rechercher un auteur..." v-model="search"
                        class="h-8 pl-8 pr-3 dark:bg-transparent dark:border dark:border-slate-200 dark:text-slate-200 w-full sm:w-48 rounded-lg bg-slate-50 border border-slate-200 text-xs focus:outline-none focus:ring-1 focus:ring-slate-300 transition-colors">
                </div>
            ...
            <div class="overflow-x-auto custom">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody v-if="!loading && filteredAuthors.length !== 0" class="text-xs">
                        <tr class="group hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors" v-for="(author, index) in filteredAuthors"
                            :key="index">
                            <td class="py-3 px-6 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p
                                            class="font-medium text-slate-900 dark:text-slate-300 group-hover:text-orange-600 transition-colors">
                                            {{ author.name }}</p>
                                        <p class="text-xs text-slate-400 dark:text-slate-300">{{ author.pseudonym }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="py-3 px-6 whitespace-nowrap">
                                <span class="text-xs font-medium text-slate-600 dark:text-slate-300">{{ author.email?.trim() ?? "---"
                                }}</span>
                            </td>
                            <td class="py-3 px-6 whitespace-nowrap">
                                <span class="text-xs font-medium text-slate-600 dark:text-slate-300">{{ author.country.name }}</span>
                            </td>
                            ...
                            <td class="py-3 px-6 text-xs text-slate-500 dark:text-slate-300 whitespace-nowrap">
                            ...
                            <td class="py-3 px-6 text-right whitespace-nowrap">
                                <select :value="author.status" @change="(e) => changeValStatus(e, author)" v-if="author.role?.name !== 'super-admin'"
