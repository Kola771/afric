              ...
              <DialogPanel
                ...
                <div class="dark:bg-dark bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                      <DialogTitle as="h3" class="text-base font-semibold text-gray-900 dark:text-white">Supprimer cet
                        auteur "<strong class="text-orange-600 dark:text-orange-500">{{ author.name }}</strong>"</DialogTitle>
                      <div class="mt-2">
                        <p class="text-sm text-gray-500 dark:text-slate-200">Êtes-vous sûr de vouloir supprimer cet
                          auteur ? Toutes les histoires liées à cet auteur seront supprimées définitivement après avoir effectué cette action.</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="dark:bg-slate-800 bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  <button type="button" :disabled="disabled"
                    ...
                    @click="functionDelete">Supprimer</button>
                  <button type="button"
...
const { completeRemoval } = usersData();
...
const emit = defineEmits(['close-delete-modal', 'on-load']);
const closeModal = () => {
  error.value = null;
  message.value = null;
...
const functionDelete = async () => {
  ...
  message.value = null;
  disabled.value = true;
  const response = await completeRemoval(props.author.uuid);
