                  ...
                  <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                      <DialogTitle as="h3" class="text-base font-semibold text-gray-900 dark:text-white">Supprimer cet
                        utilisateur "<strong class="text-orange-600 dark:text-orange-500">{{ user.name }}</strong>"
                      </DialogTitle>
                      <div class="mt-2">
                        <p class="text-sm text-gray-500 dark:text-slate-200">Êtes-vous sûr de vouloir supprimer cet
                          utilisateur ? Toutes les données liées à cet utilisateur seront supprimées définitivement après avoir effectué cette action.</p>
                      </div>
                    </div>
                  </div>
                </div>
...
const functionDelete = async () => {
  ...
  message.value = null;
  disabled.value = true;
  const response = await completeRemoval(props.user.uuid);
