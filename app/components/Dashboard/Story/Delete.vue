                ...
                <div class="dark:bg-dark bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                      <DialogTitle as="h3" class="text-base font-semibold text-gray-900 dark:text-white">Supprimer cette
                        histoire "<strong class="text-orange-600 dark:text-orange-500">{{ book.title }}</strong>"
                      </DialogTitle>
                      <div class="mt-2">
                        <p class="text-sm text-gray-500 dark:text-slate-200">Êtes-vous sûr de vouloir supprimer cette
                          histoire ? Elle sera supprimée définitivement avec toutes les données qui sont reliées à elle
                          après avoir effectué cette action.</p>
                      </div>
                    </div>
                  </div>
                </div>
...
const functionDelete = async () => {
  ...
  message.value = null;
  disabled.value = true;
  const response = await deleteData(props.book.uuid);
