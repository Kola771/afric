        ...
        <div class="flex flex-col items-start">
            ...
            <div class="w-full h-72 mt-2 relative">
                <img src="/assets/img2.jpg"
                    class="w-full h-full object-cover grayscale-[20%] rounded-lg transition-transform duration-700"
                    alt="Image de couverture de l'histoire">
                <nuxt-link to="/dashboard/stories/story-uuid-14/detail-like-comment"
                    class="hidden lg:flex items-center gap-4 absolute bottom-4 right-4 bg-white p-2 rounded-lg">
                    <span class="text-xs">
                        <span class="font-semibold text-slate-900 dark:text-slate-800">42</span> <span
                            class="text-slate-400 dark:text-slate-700">
                            <Icon name="mdi:heart" class="w-4 h-4" />
                        </span>
                    </span>
                    <span class="w-px h-3 bg-slate-200"></span>
                    <span class="text-xs">
                        <span class="font-semibold text-slate-900 dark:text-slate-800">12k</span> <span
                            class="text-slate-400 dark:text-slate-700">
                    ...
                    <span class="w-px h-3 bg-slate-200"></span>
                    <span class="text-xs">
                        <span class="font-semibold text-slate-900 dark:text-slate-800">1.1k</span> <span
                            class="text-slate-400 dark:text-slate-700">
            ...
            <div class="mt-3 flex flex-col gap-2">
                <div class="flex items-center gap-3 uppercase text-xs font-medium text-orange-600">
                    <span
                        class="bg-orange-50 border border-orange-100/50 text-orange-600 text-xs font-medium animate-fade-in-up px-2 py-1 rounded">Roman</span>
                    <span
                        class="bg-orange-50 border border-orange-100/50 text-orange-600 text-xs font-medium animate-fade-in-up px-2 py-1 rounded">Thriller</span>
                    <span class="bg-slate-100 text-slate-500 px-2 py-1 rounded">Terminée</span>
                </div>
                <nuxt-link to="/dashboard/stories/story-uuid-14/detail-like-comment"
                    class="lg:hidden flex items-center gap-4">
                    <span class="text-xs">
            ...
            <div class="py-4 border-b border-slate-200">
                <h2 class="text-xl font-display font-bold text-slate-900 dark:text-white tracking-tight">Les Larmes du
                    Soleil</h2>
                <p class="text-sm text-slate-500 dark:text-slate-200 mt-1">Lorem ipsum dolor sit amet consectetur
                    adipisicing elit. Magnam ea ducimus distinctio libero, quae quasi laboriosam autem delectus!
                    Perferendis neque et necessitatibus officia assumenda? Ratione!</p>
            </div>
            <div class="flex flex-col lg:flex-row lg:gap-4 lg:justify-between w-full pt-4">
                ...
                <div class="hidden lg:block lg:w-1/4 space-y-2 max-h-[50vh] overflow-y-auto">
                    <button v-for="chap in chapters" :key="chap.id" @click="() => { attemptSwitchChapter(chap.id); }"
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex justify-between items-center mb-0.5">
                                <span
                                    ...
                                    chap.title }}</span>
                                <span
                                    :class="['text-[10px] font-semibold px-1.5 py-0.5 rounded border', chap.status === 'Publié' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-slate-100 text-slate-500 border-slate-200']">{{
                                    chap.status }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <p
                                    :class="['text-xs truncate font-serif', chap.id === selectedChapterId ? 'text-slate-500' : 'text-slate-400']">
                                    {{ chap.subtitle }}</p>
                                <div class="flex items-center gap-1">
                                    <div class="text-xs">
                                        <span
                                            class="font-semibold text-slate-900 dark:text-slate-400 dark:group-hover:text-slate-700">{{
                                            chap.like || 0 }}</span> <span class="text-slate-400 dark:text-slate-600">
                                            <Icon name="mdi:heart" class="w-4 h-4" />
                                        </span>
                                    </div>
                                    <div class="text-xs">
                                        <span
                                            class="font-semibold text-slate-900 dark:text-slate-400 dark:group-hover:text-slate-700">{{
                                            chap.view || 0 }}</span> <span class="text-slate-400 dark:text-slate-600">
                                            <Icon name="mdi:eye" class="w-4 h-4" />
                                        </span>
                                    </div>
                ...
                <div v-if="selectedChapter" class="mt-4 lg:flex-1 lg:mt-0 bg-white lg:bg-slate-100 rounded-lg">
                    <div class="p-2 text-[13px] lg:text-xs flex flex-wrap items-center gap-x-1 gap-y-2.5">
                        <button @click="showChapter"
                            :class="step === 'chapter' ? 'bg-slate-50 dark:bg-slate-200 dark:hover:bg-slate-300' : 'bg-white dark:hover:bg-slate-50'"
                            class="px-4 py-2 rounded dark:border-slate-400 dark:border flex items-center gap-2 transition-all duration-300">
                        ...
                        <button @click="showComments"
                            ...
                            {{ selectedChapter.comment || 0 }} Commentaires
                        </button>
                    </div>
                    ...
                    <div class="p-4" v-if="step === 'likes'">
                        ...
                        <div
                            class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-4 mt-4 max-h-96 overflow-y-auto">
