<template>
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-xl font-display font-bold text-slate-900 dark:text-white tracking-tight">Gestion des Pays</h1>
                <p class="text-sm text-slate-500 dark:text-slate-200 mt-1">Gérez les pays et suivez leurs performances.</p>
            </div>
            <div class="flex gap-3">
                    <button @click="exportCountries" class="h-9 px-4 rounded-lg bg-white dark:bg-slate-100 border border-slate-200 text-xs font-medium text-slate-700 hover:bg-slate-50 shadow-sm transition-colors flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div class="bg-white dark:bg-slate-100 p-4 rounded-xl border border-slate-200 shadow-sm">
                ...
                <div class="flex items-end justify-between">
                    <p class="text-2xl font-display font-bold text-slate-900">{{ filteredCountries.length }}</p>
                    <Icon name="mdi:globe" class="text-slate-300" width="24" />
                </div>
            </div>
            <div class="bg-white dark:bg-slate-100 p-4 rounded-xl border border-slate-200 shadow-sm">
                <p class="text-xs font-medium text-slate-500 mb-1">Total Pays actifs</p>
                <div class="flex items-end justify-between">
            ...
            <div class="bg-white dark:bg-slate-100 p-4 rounded-xl border border-slate-200 shadow-sm">
                <p class="text-xs font-medium text-slate-500 mb-1">Total Pays inactifs</p>
                <div class="flex items-end justify-between">
            ...
            <div class="bg-white dark:bg-slate-100 p-4 rounded-xl border border-slate-200 shadow-sm">
                ...
                <div class="flex items-end justify-between">
                    <p class="text-xs text-slate-500">Des auteurs proviennent du/de Bénin</p>
            ...
            <div class="bg-white dark:bg-slate-100 p-4 rounded-xl border border-slate-200 shadow-sm">
                ...
                <div class="flex items-end justify-between">
                    <p class="text-xs text-slate-500">Des auteurs proviennent du/de Sénégal</p>
        ...
        <div class="bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
            ...
            <div class="flex items-center gap-3 w-full">
                <div class="relative flex-1">
                    ...
                    <input type="search" v-model="searchQuery" @keydown.enter="executeSearch" placeholder="Rechercher un pays..." class="h-8 pl-8 pr-3 w-full rounded-lg bg-slate-50 border border-slate-200 text-xs focus:outline-none focus:ring-1 focus:ring-slate-300 transition-colors">
                </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="overflow-x-auto lg:max-h-96 lg:overflow-y-auto custom">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-sm">
                        <tr class="group hover:bg-slate-50 transition-colors border-b border-slate-50" v-for="(country, index) in sortedCountries" :key="index">
                            <td class="py-3 px-6">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-10 bg-slate-200 rounded object-cover overflow-hidden">
                                        <img :src="`${config.public.apiBackendUrl}/uploads/countries/${country.flag}`" class="w-full h-full object-cover opacity-80">
                                    </div>
                                    <span class="font-medium text-slate-900 group-hover:text-orange-600 transition-colors">{{ country.name }}</span>
                                </div>
                            </td>
                            <td class="py-3 px-6 text-slate-600 text-xs">{{ country.code }}</td>
                            <td class="py-3 px-6 text-slate-600 text-xs">{{ country.prefix }}</td>
                            <td class="py-3 px-6 text-slate-600 text-xs">{{ country.language }}</td>
                            <td class="py-3 px-6 whitespace-nowrap">
                                <span :class="`capitalize inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-medium ${country.status === 'actif' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'} `">
                                    <span :class="`w-1 h-1 rounded-full ${country.status === 'actif' ? 'bg-green-600' : 'bg-red-600'}`"></span>
                                    {{ country.status }}
                                </span>
                            </td>
                            <td class="py-3 px-6 whitespace-nowrap">
                                <span :class="`inline-flex items-center gap-1.5 text-[10px] font-medium ${country.status === 'actif' ? ' text-green-700' : 'text-red-700'} `">
                                    <span :class="`w-1 h-1 rounded-full ${country.status === 'actif' ? 'bg-green-600' : 'bg-red-600'}`"></span>
                                    {{ formatLocalDate(country.deadline || '') }}
                                </span>
                            </td>
                            <td class="py-3 px-6 text-right whitespace-nowrap">
                                <nuxt-link :to="`/dashboard/countries/edit/${country.uuid}`" v-if="country.status === 'actif'" class="ml-2 text-orange-600 hover:text-orange-700 transition-colors"><Icon name="mdi:edit" class="w-4 h-4" /></nuxt-link>
                                <button @click="toggleDeleteModal(country)" v-if="country.status === 'actif'" class="ml-2 text-red-600 hover:text-red-700 transition-colors"><Icon name="mdi:trash" class="w-4 h-4" /></button>
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="flex items-center justify-between p-4 border-t border-slate-100">
                <span class="text-xs font-bold text-slate-500">{{ filteredCountries.length }} pays</span>
            </div>
        </div>
        <DashboardCountryDelete @close-delete-modal="closeDeleteModal" :country="country" :showDeleteModal="showDeleteModal" v-if="country && showDeleteModal" />
    </div>
</template>
